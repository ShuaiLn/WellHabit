...
def add_task():
    ...
    task_date = _parse_date(request.form.get('task_date'), fallback=local_today())
...
def save_ai_suggestion_followup(task_id):
    ...
    followup_context = 'pattern support' if (task.ai_generated_source or '') == 'pattern_recognition' else 'negativity detected'
    latest_event = f'AI suggestion follow-up after {followup_context}: felt better {rating}/10 after {task.title}'
    payload = _apply_wellness_update(current_user, local_today(), latest_event)
    _log_activity_entry(
        current_user.id,
        'care',
        'AI suggestion follow-up saved',
        f'{task.title} · feel better {rating}/10 after {followup_context}',
