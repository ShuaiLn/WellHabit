...
def _resolve_secret_key() -> tuple[str, bool]:
    ...
    raise RuntimeError('SECRET_KEY is required when FLASK_DEBUG is not 1.')
...
def create_app():
    ...
    def apply_security_headers(response):
        ...
        response.headers.setdefault(
            ...
            "default-src 'self'; script-src 'self' 'nonce-{}'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self' data:; connect-src 'self'; frame-src 'self' https://www.youtube.com https://www.youtube-nocookie.com; object-src 'none'; base-uri 'self'; form-action 'self'".format(nonce),
        )
